...
select 
    item_purchased,
    round(avg(cast(review_rating as decimal(10,2))), 2) as avg_review_rating
from 
    customer
group by 
    item_purchased
...
select   subscription_status,avg(purchase_amount),sum(purchase_amount),count(customer_id) from customer group by subscription_status;



-- Q6. Which 5 products have the highest percentage of purchases with discounts applied?

select item_purchased,round(100.0 * sum(case when discount_applied = 'Yes' then 1 else 0 end) / count(*),2) as rate
from customer group by item_purchased order by rate desc limit 5;
